...
final class MacTokenTest extends TestCase
{
    ...
    public function genericCalls(): void
    {
        ...
        static::assertSame('MAC', $macToken->name());
        static::assertSame('MAC', $macToken->getScheme());
        static::assertSame([
            'mac_key' => 'MAC_KEY_FOO_BAR',
            'mac_algorithm' => 'hmac-sha-256',
        ], $macToken->getAdditionalInformation());
    }
    ...
    public function anAccessTokenInTheAuthorizationHeaderIsFound(): void
    {
        ...
        $request->getHeader('AUTHORIZATION')
            ->willReturn(['MAC id="h480djs93hd8",ts="1336363200",nonce="dj83hs9s",mac="bhCQXTVyfj5cmA9uKkPFx1zeOXM="'])
        ;

        $additionalCredentialValues = [];
        static::assertSame('h480djs93hd8', $macToken->find($request->reveal(), $additionalCredentialValues));
        static::assertSame([
            'id' => 'h480djs93hd8',
            'ts' => '1336363200',
            'nonce' => 'dj83hs9s',
    ...
    public function iFoundAValidAccessToken(): void
    {
        ...
        $request->getRequestTarget()
            ->willReturn('/request?b5=%3D%253D&a3=a&c%40=&a2=r%20b&c2&a3=2+q')
        ;

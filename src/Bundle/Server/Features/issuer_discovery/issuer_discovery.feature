Feature: An Isuer Discovery Endpoint is available
  ...
  Scenario: A client sends an Issuer Discovery request without "rel" parameter
  ...
  Scenario: A client sends an Issuer Discovery request with an invalid "rel" parameter
  ...
  Scenario: A client sends an Issuer Discovery request without "resource" parameter
  ...
  Scenario: A client sends an Issuer Discovery request with an invalid resource parameter based on an XRI
  ...
  Scenario: A client sends an Issuer Discovery request with an invalid resource parameter based on an email
  ...
  Scenario: A client sends an Issuer Discovery request with an invalid resource parameter based on an Url
    When A client sends an Issuer Discovery request with an invalid resource parameter based on an Url
    Then the response contains an error with code 400
    And the error is "invalid_request"
    And the error description is "Unsupported domain."

  Scenario: A client sends an Issuer Discovery request with a valid resource parameter based on an email

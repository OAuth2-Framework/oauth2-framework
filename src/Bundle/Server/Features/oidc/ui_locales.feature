Feature: A client requests an authorization
  ...
  Scenario: A client sends an authorization request with a ui_locales parameter.
  ...
  Scenario: A client sends an authorization request with a ui_locales parameter but none of them is supported.
    Given The user "john.1" is logged in and but not fully authenticated

...
class OAuth2SecurityFactory implements SecurityFactoryInterface
{
    ...
    {
        $node->children()
            ->scalarNode('handler_manager')
                ->info('The access token handler manager retrieves the access tokens on demand. Access token can be find from a database, the introspection or any other method.')
                ->isRequired()
            ->end()
        ->end();
    }

...
use OAuth2Framework\ServerBundle\DependencyInjection\Compiler\HttpClientCompilerPass;
...
class OAuth2FrameworkServerBundle extends Bundle
{
    ...
    private function loadDoctrineSchemas(ContainerBuilder $container): void
    {
        ...
        $map = [
            realpath(
                __DIR__ . '/Resources/config/doctrine-mapping/AccessToken'
            ) => 'OAuth2Framework\Component\Core\AccessToken',
        ...
        if (class_exists(AbstractAuthorizationCode::class)) {
            $map[realpath(
                __DIR__ . '/Resources/config/doctrine-mapping/AuthorizationCodeGrant'
            )] = 'OAuth2Framework\Component\AuthorizationCodeGrant';
        }
        if (class_exists(AbstractInitialAccessToken::class)) {
            $map[realpath(
                __DIR__ . '/Resources/config/doctrine-mapping/ClientRegistrationEndpoint'
            )] = 'OAuth2Framework\Component\ClientRegistrationEndpoint';
        }
        if (class_exists(AbstractRefreshToken::class)) {
            $map[realpath(
                __DIR__ . '/Resources/config/doctrine-mapping/RefreshTokenGrant'
            )] = 'OAuth2Framework\Component\RefreshTokenGrant';
        }

...
class UserinfoEndpointEncryptionCompilerPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container): void
    {
        if (! $container->hasDefinition(UserInfoEndpoint::class) || ! $container->hasDefinition(
            'jose.jwe_builder.oauth2_server.openid_connect.id_token_from_userinfo'
        )) {
            return;
        }
        ...
        $definition->addMethodCall(
            'enableEncryption',
            [new Reference('jose.jwe_builder.oauth2_server.openid_connect.id_token_from_userinfo')]
        );
        ...
        if ($container->hasDefinition(MetadataBuilder::class)) {
            ...
            $definition->addMethodCall(
                'addKeyValuePair',
                ['userinfo_encryption_alg_values_supported', $container->getParameter(
                    'oauth2_server.openid_connect.userinfo_endpoint.encryption.key_encryption_algorithms'
                )]
            );
            $definition->addMethodCall(
                'addKeyValuePair',
                ['userinfo_encryption_enc_values_supported', $container->getParameter(

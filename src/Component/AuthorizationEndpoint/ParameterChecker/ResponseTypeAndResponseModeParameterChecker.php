...
final class ResponseTypeAndResponseModeParameterChecker implements ParameterChecker
{
    ...
    public function check(AuthorizationRequest $authorization): void
    {
        ...
        Assertion::true($authorization->getClient()->isResponseTypeAllowed($responseTypeName), \Safe\sprintf('The response type "%s" is not allowed for this client.', $responseTypeName)); // Should try to find the response mode before exception
        ...
        Assertion::true($this->responseModeManager->has($responseModeName), \Safe\sprintf('The response mode "%s" is not supported. Please use one of the following values: %s.', $responseModeName, \implode(', ', $this->responseModeManager->list())));
    ...
    {
        Assertion::true($this->responseTypeManager->has($responseType), \Safe\sprintf('The response type "%s" is not supported by this server', $responseType));

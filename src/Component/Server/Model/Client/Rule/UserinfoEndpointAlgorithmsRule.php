...
final class UserinfoEndpointAlgorithmsRule implements RuleInterface
{
    ...
    public function handle(DataBag $commandParameters, DataBag $validatedParameters, ? UserAccountId $userAccountId, callable $next): DataBag
    {
        if ($commandParameters->has('userinfo_signed_response_alg') && null !== $this->jwsBuilder) {
            ...
            Assertion::inArray($commandParameters['userinfo_signed_response_alg'], $this->jwsBuilder->getSupportedSignatureAlgorithms(), sprintf('The ID Token signature response algorithm \'%s\' is not supported. Please choose one of the following algorithm: %s', $commandParameters->get('userinfo_signed_response_alg'), implode(', ', $this->jwsBuilder->getSupportedSignatureAlgorithms())));
        ...
        if ($commandParameters->has('userinfo_encrypted_response_alg') && $commandParameters->has('userinfo_encrypted_response_enc') && null !== $this->jweBuilder) {
            ...
            Assertion::inArray($commandParameters['userinfo_encrypted_response_alg'], $this->jweBuilder->getSupportedKeyEncryptionAlgorithms(), sprintf('The ID Token content encryption algorithm \'%s\' is not supported. Please choose one of the following algorithm: %s', $commandParameters->get('userinfo_encrypted_response_alg'), implode(', ', $this->jweBuilder->getSupportedContentEncryptionAlgorithms())));
            Assertion::inArray($commandParameters['userinfo_encrypted_response_enc'], $this->jweBuilder->getSupportedContentEncryptionAlgorithms(), sprintf('The ID Token key encryption algorithm \'%s\' is not supported. Please choose one of the following algorithm: %s', $commandParameters->get('userinfo_encrypted_response_enc'), implode(', ', $this->jweBuilder->getSupportedKeyEncryptionAlgorithms())));
